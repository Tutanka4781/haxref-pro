...
function _updateScoreLogos(){
  // En la UI de la app los logos siguen al swap (redName puede ser el equipo original azul)
  const swapped   = state.redName !== state.origRed;
  const logoLeft  = swapped ? state.shieldBlue : state.shieldRed;
  const logoRight = swapped ? state.shieldRed  : state.shieldBlue;
  ...
  if(imgR && phR){
    if(logoLeft){ imgR.src=logoLeft; imgR.style.display=''; phR.style.display='none'; }
    else        { imgR.style.display='none'; phR.style.display=''; }
  }
  if(imgB && phB){
    if(logoRight){ imgB.src=logoRight; imgB.style.display=''; phB.style.display='none'; }
