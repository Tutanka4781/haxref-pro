...
function _showLigaPicker(ligaList, onConfirm, onCancel){
  ...
  backdrop.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:2000;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px);animation:backdropIn .18s ease both';

  let selectedLiga = ligaList[0];
  let redTeam  = null;
  let blueTeam = null;

  function close(){ backdrop.remove(); _lpCleanup(); }

  function render(){
    backdrop.innerHTML = `
      <div style="background:#161616;border:1px solid #2e2e2e;border-radius:18px;width:340px;max-height:85vh;display:flex;flex-direction:column;overflow:hidden;font-family:var(--font-display);animation:sheetSlideUp .22s cubic-bezier(.34,1.56,.64,1) both">
        <div style="padding:18px 20px 12px;border-bottom:1px solid #1e1e1e;display:flex;align-items:center;justify-content:space-between">
          <div style="font-size:11px;font-weight:900;letter-spacing:2px;color:#555">INICIO RÁPIDO</div>
          <button id="lp-close-btn" style="background:transparent;border:none;color:#444;font-size:16px;cursor:pointer;line-height:1">✕</button>
        </div>
        <div style="padding:12px 20px 6px">
          <div style="font-size:9px;color:#444;letter-spacing:1.5px;margin-bottom:6px">LIGA</div>
          <div style="display:flex;gap:6px;flex-wrap:wrap">
            ${ligaList.map(l=>`
              <button data-liga-id="${l.id}" style="padding:6px 12px;border-radius:7px;font-size:10px;font-weight:900;letter-spacing:1px;cursor:pointer;border:1px solid ${l.id===selectedLiga.id?'#4a2a6a':'#222'};background:${l.id===selectedLiga.id?'#0d0d18':'transparent'};color:${l.id===selectedLiga.id?'#c4b5fd':'#555'};transition:.15s">${l.name}</button>
            `).join('')}
          </div>
        </div>
        ...
        <div style="padding:12px 20px;border-top:1px solid #1e1e1e;display:flex;gap:8px">
          <button id="lp-manual-btn" style="flex:1;padding:10px;background:#1a1a1a;border:1px solid #222;color:#555;border-radius:9px;font-size:10px;font-weight:900;letter-spacing:1px;cursor:pointer;font-family:var(--font-display)">MANUAL</button>
          <button id="lp-confirm-btn" style="flex:2;padding:10px;background:linear-gradient(135deg,#3a1a5a,#5a2a8a);border:1px solid #5a2a8a;color:#e9d5ff;border-radius:9px;font-size:10px;font-weight:900;letter-spacing:1px;cursor:pointer;font-family:var(--font-display);${!redTeam||!blueTeam?'opacity:.4;pointer-events:none':''}">INICIAR →</button>
        </div>
      </div>
    `;
...
function _lpRenderTeamCol(label, teams, selected, side){
  return `
    <div style="flex:1;display:flex;flex-direction:column;overflow:hidden;border-right:${side==='red'?'1px solid #1e1e1e':'none'}">
      <div style="padding:10px 14px 6px;font-size:9px;color:#444;letter-spacing:1.5px;flex-shrink:0">${label}</div>
      <div style="overflow-y:auto;flex:1;padding:0 8px 10px;scrollbar-width:thin;scrollbar-color:#222 transparent">
        ${teams.map(t=>`
          <div data-team-id="${t.id}" data-side="${side}" style="display:flex;align-items:center;gap:8px;padding:7px 8px;border-radius:8px;cursor:pointer;border:1px solid ${selected?.id===t.id?'#'+(side==='red'?'6b1a1a':'1a2a6b'):'transparent'};background:${selected?.id===t.id?'#'+(side==='red'?'1a0808':'080818'):'transparent'};margin-bottom:2px;transition:.12s">
            <div style="width:28px;height:28px;border-radius:5px;overflow:hidden;background:#0a0a0a;flex-shrink:0;display:flex;align-items:center;justify-content:center">
              ${t.logo?`<img src="${t.logo}" style="width:100%;height:100%;object-fit:contain">`:'<span style="font-size:12px;opacity:.3">🛡</span>'}
            </div>
            <span style="font-size:10px;font-weight:700;color:${selected?.id===t.id?(side==='red'?'#ff9999':'#9999ff'):'#777'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${t.name}</span>
          </div>
        `).join('')}
      </div>
    </div>`;
...
function renameTeams(){
  showModal('NUEVO NOMBRE — EQUIPO ROJO (Enter para dejar igual)',val=>{
    if(val && val.trim()){
      const oldRed=state.redName;const newRed=val.trim();
      state.events.forEach(ev=>{if(ev.teamName===oldRed)ev.teamName=newRed;});
      state.redName=newRed;state.origRed=newRed;
      document.getElementById('lbl-red').textContent=newRed;
    }
    setTimeout(()=>{
      showModal('NUEVO NOMBRE — EQUIPO AZUL (Enter para dejar igual)',val2=>{
        if(val2 && val2.trim()){
          const oldBlue=state.blueName;const newBlue=val2.trim();
          state.events.forEach(ev=>{if(ev.teamName===oldBlue)ev.teamName=newBlue;});
          state.blueName=newBlue;state.origBlue=newBlue;
          document.getElementById('lbl-blue').textContent=newBlue;
        }
...
function _updateScoreLogos(){
  ...
  if(imgR && phR){
    if(logoL){ imgR.src=logoL; imgR.style.display=''; phR.style.display='none'; }
    else     { imgR.style.display='none'; phR.style.display=''; }
  }
  if(imgB && phB){
    if(logoR){ imgB.src=logoR; imgB.style.display=''; phB.style.display='none'; }
...
function openCardModal(){
  ...
  document.getElementById('card-team-modal').style.display = 'flex';
}
...
function openCardList(){
  ...
  renderEventList();
  document.getElementById('card-list-modal').style.display = 'flex';
}
...
function _checkResumeMatch() {
  ...
  resumeBlock.innerHTML = `
    <div style="font-size:9px;letter-spacing:2px;color:#7c3aed;margin-bottom:6px">PARTIDO EN CURSO</div>
    ...
    <div style="font-size:10px;color:#7c3aed;letter-spacing:1px">▶ CONTINUAR</div>
  `;
...
function openNavPicker(){ document.getElementById('nav-picker').classList.add('open'); }
function closeNavPicker(){ document.getElementById('nav-picker').classList.remove('open'); }
  ...
  function onMessage(msg) {
    switch (msg.type) {
      ...
      case 'game_started': {
        ...
        // Si HaxRef ya tiene partido iniciado → es el 2T, sincronizar timer a 0
        if (state.inProgress && !state.endTime) {
          state.matchTimer = 0;
          updatePeriodUI();
          updateLiveScoreboard();
          console.log('[HaxRef Live] 2T detectado — timer a 0');
        }
        break;
      }
      ...
      case 'game_stopped': {
        ...
        // Pedir confirmación de ½T solo si:
        // - HaxRef tiene partido activo
        // - Era el primer tiempo
        if (state.inProgress && !state.endTime && wasFirstHalf) {
          _showHalftimeConfirmation();
        }
        break;
      }
